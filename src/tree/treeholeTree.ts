...
export class TreeholeTreeDataProvider extends AbstractTreeDataProvider {
    ...
    public async getItems(): Promise<Node[]> {
        const response: any = await treeholeApi.getTreeholes(this.currentPage);
        ...
        data.list.forEach((element: any) => {
            ...
            const node = new Node(
                treeholeItem.comment_author,
                vscode.TreeItemCollapsibleState.None,
                {
                    command: 'jandan.select',
                    title: '',
                    arguments: ['treehole', treeholeItem],
                },
            );
            node.id = `treehole-${treeholeItem.comment_ID}`;
            items.push(node);
        });
        return items;
    }
